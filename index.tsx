...
const uploadToCloudinary = async (file: File): Promise<string> => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error?.message || 'Gagal upload.');
    }

    const data = await response.json();
    return data.secure_url;
  } catch (err: any) {
    throw new Error(err.message || 'Koneksi bermasalah.');
  }
};
  ...
  const generateAICaption = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [{
          parts: [
            { text: "Berikan 1 kutipan pendek puitis Bahasa Indonesia (maks 10 kata) untuk foto ini." },
            { inlineData: { mimeType: 'image/jpeg', data: base64Data } }
          ]
        }]
      });

      const caption = response.text || "Kenangan indah.";
      ...
      <div className="fixed inset-0 flex items-center justify-center p-6 bg-sky-500">
        ...
        <div className="w-full max-w-md space-y-12 text-center z-10 animate-in fade-in zoom-in">
          ...
          <div className="space-y-4">
            ...
            <p className="text-[10px] text-white/70 font-black uppercase tracking-[0.3em]">Brankas Kenangan Tanpa Password</p>
          </div>
          <form onSubmit={handleGate} className="p-8 bg-white/90 backdrop-blur-3xl border border-white rounded-[3rem] shadow-2xl space-y-6">
            ...
            <button type="submit" className="w-full py-5 bg-sky-500 text-white font-black rounded-[2rem] shadow-xl hover:bg-sky-600 active:scale-95 transition-all flex items-center justify-center gap-3"><Zap className="w-4 h-4" /> Masuk Sekarang</button>
          </form>
        </div>
      </div>
    );
        ...
        <div className="max-w-7xl mx-auto flex flex-col gap-5 items-center">
          <div className="flex w-full justify-between items-center">
            ...
            <div className="flex items-center gap-2">
               <button onClick={() => fetchIndex()} className={`p-2.5 text-sky-400 ${isSyncing ? 'animate-spin' : ''}`} title="Sinkronkan data">
                 <RefreshCw className="w-5 h-5" />
               </button>
               <button onClick={() => { sessionStorage.clear(); window.location.reload(); }} className="p-2.5 text-red-400" title="Keluar">
        ...
        <div className="fixed inset-0 z-[60] bg-sky-900/60 backdrop-blur-3xl flex flex-col animate-in fade-in">
           ...
           <div className="w-full bg-white rounded-t-[3rem] p-8 space-y-6 shadow-2xl flex flex-col max-h-[60vh] overflow-hidden relative">
              ...
              <div className="flex gap-3 mt-auto pt-2">
                 <input type="text" value={newComment} onChange={(e) => setNewComment(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addComment()} placeholder="Tulis pesan..." className="flex-1 px-5 py-4 bg-sky-50 border border-sky-100 rounded-2xl outline-none text-xs" />
