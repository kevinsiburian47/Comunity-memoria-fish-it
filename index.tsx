...
// Initialize the Google GenAI client using the required configuration
...
// --- Database Config (Multi-Key Upgrade) ---
const BUCKET_ID = 'memoria_rasi_bintang_v4_upgraded';
...
const compressImage = (base64: string): Promise<string> => {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      const MAX_SIZE = 450; // Dioptimalkan agar 10+ foto muat dengan aman
      let width = img.width;
      ...
      resolve(canvas.toDataURL('image/jpeg', 0.4)); // Kualitas 40% sangat ideal untuk DB cloud gratis
    };
  });
};
...
const App = () => {
  ...
  const [showModal, setShowModal] = useState<{show: boolean, type: 'create' | 'login' | 'gate'}>({ show: !sessionStorage.getItem('isMemoriaGuest'), type: 'gate' });
  ...
  const saveIndex = async (data: AlbumMetadata[]) => {
    try {
      ...
      alert("Gagal memperbarui daftar album.");
    }
  };
  ...
  const saveAlbumPhotos = async (albumId: string, photos: Photo[]) => {
    try {
      ...
      if (!res.ok) throw new Error("Database Full/Error");
      setActivePhotos(photos);
      
      // Update count di index
      const newIndex = albums.map(a => a.id === albumId ? { ...a, photoCount: photos.filter(p => !p.deletedAt).length } : a);
      saveIndex(newIndex);
    } catch (e) {
      alert("Gagal menyimpan foto. Coba kurangi jumlah foto yang diunggah sekaligus.");
    }
  };
  ...
  // Fixed handleAddPhotos: Explicitly cast Array.from result to File[] to avoid 'unknown' assignability errors to Blob parameters
  ...
  const handleArchive = async (id: string, type: 'album' | 'photo') => {
    if (!isAdmin) return;
    if (type === 'album') {
      const next = albums.map(a => a.id === id ? { ...a, deletedAt: Date.now() } : a);
      saveIndex(next);
    } else if (activeAlbumId) {
      const next = activePhotos.map(p => p.id === id ? { ...p, deletedAt: Date.now() } : p);
  ...
  const filteredAlbums = useMemo(() => 
    albums.filter(a => !a.deletedAt && a.name.toLowerCase().includes(searchTerm.toLowerCase())), 
    [albums, searchTerm]
  );
        ...
        <div className="w-full max-w-md space-y-12 text-center z-10 animate-in fade-in zoom-in duration-700">
          ...
          <form onSubmit={handleGate} className="p-10 bg-white/80 backdrop-blur-3xl border border-white rounded-[3.5rem] shadow-2xl space-y-6">
            ...
            <button type="submit" className="w-full py-5 bg-sky-500 text-white font-black rounded-[2rem] shadow-xl hover:bg-sky-600 active:scale-95 transition-all uppercase tracking-widest text-xs flex items-center justify-center gap-3">
              <Zap className="w-4 h-4" /> Buka Cakrawala
            </button>
          </form>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex items-center gap-4">
            <div className={`hidden lg:flex items-center gap-2 px-4 py-2 bg-white/40 rounded-2xl border ${syncStatus === 'error' ? 'border-red-300' : 'border-white/60'}`}>
              {/* Using Cloud icon instead of non-existent CloudCheck */}
              {syncStatus === 'syncing' ? <RefreshCw className="w-3 h-3 text-sky-500 animate-spin" /> : <Cloud className="w-3 h-3 text-sky-500" />}
              <span className="text-[9px] font-black uppercase text-sky-800/40">Realtime</span>
            </div>
            {isAdmin && (
              <button onClick={() => setCurrentView(currentView === 'archive' ? 'home' : 'archive')} className={`p-3 rounded-2xl border transition-all ${currentView === 'archive' ? 'bg-sky-600 text-white' : 'bg-white/40 text-sky-600 border-white/60 hover:bg-white'}`}>
            ...
            {currentView === 'home' && isAdmin && (
              <button onClick={() => { setInputVal(''); setShowModal({show: true, type: 'create'}); }} className="flex items-center gap-3 bg-sky-500 text-white px-6 py-3.5 rounded-2xl shadow-lg font-black uppercase text-[10px]">
          ...
          <div className="space-y-12 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-10">
              {archivedAlbums.map(album => (
                <div key={album.id} className="bg-white/40 rounded-[2.5rem] overflow-hidden shadow-xl border border-white opacity-80 group hover:opacity-100 transition-all p-8 text-center">
                  <h3 className="font-serif font-black text-sky-900 truncate uppercase mb-4">{album.name}</h3>
                  <button onClick={() => {
                    const next = albums.map(a => a.id === album.id ? { ...a, deletedAt: undefined } : a);
                    saveIndex(next);
                  }} className="bg-white text-sky-600 p-3 rounded-full shadow-xl"><RotateCcw className="w-5 h-5" /></button>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-16 animate-in fade-in duration-700">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 text-white drop-shadow-lg">
              <div>
                ...
                <p className="text-base font-medium opacity-80">Orbit yang menyimpan kenangan abadi rasi bintang kita.</p>
              </div>
              <div className="px-5 py-3 bg-white/20 rounded-2xl border border-white text-[10px] font-black uppercase tracking-widest flex items-center gap-3">
                < Zap className="w-4 h-4 fill-current" /> {filteredAlbums.length} Collections
              </div>
            </div>
              ...
              {filteredAlbums.map((album, idx) => (
                ...
                >
                  <div className="aspect-[4/5] bg-sky-50 relative flex items-center justify-center text-sky-200">
                    ...
                    {isAdmin && (
                      <button onClick={(e) => { e.stopPropagation(); handleArchive(album.id, 'album'); }} className="absolute top-6 right-6 p-3 bg-white text-red-500 rounded-2xl opacity-0 group-hover:opacity-100 transition-all shadow-xl"><Trash2 className="w-5 h-5" /></button>
                    )}
                  </div>
          ...
          <div className="space-y-12 animate-in fade-in duration-500">
            ...
            {isLoadingAlbum ? (
              ...
              <div className="columns-2 sm:columns-3 lg:columns-4 gap-10 space-y-10">
                {activePhotos.filter(p => !p.deletedAt).map((photo, index) => (
                  <div key={photo.id} className="relative group break-inside-avoid bg-white/70 backdrop-blur-3xl rounded-[3rem] overflow-hidden shadow-2xl border border-white transition-all hover:border-sky-400 animate-in fade-in duration-700">
                    ...
                    {isAdmin && (
                      <button onClick={(e) => { e.stopPropagation(); handleArchive(photo.id, 'photo'); }} className="absolute top-5 right-5 p-3 bg-white/80 text-red-500 rounded-2xl opacity-0 group-hover:opacity-100 transition-all shadow-lg"><Trash2 className="w-4 h-4" /></button>
                    )}
      ...
      {selectedPhotoIndex !== null && activePhotos[selectedPhotoIndex] && (
        <div className="fixed inset-0 z-[60] bg-sky-900/40 backdrop-blur-[50px] flex flex-col md:flex-row animate-in fade-in duration-300">
           ...
           <div className="flex-1 flex items-center justify-center p-12">
              <img src={activePhotos[selectedPhotoIndex].url} className="max-w-full max-h-full object-contain rounded-[3rem] shadow-2xl border-[10px] border-white/50" />
           </div>
              ...
              <div className="flex-1 overflow-y-auto p-10 space-y-8">
                 {activePhotos[selectedPhotoIndex].comments?.map(c => (
                    <div key={c.id} className="flex gap-5 animate-in slide-in-from-right">
                       <div className="w-10 h-10 rounded-2xl bg-sky-100 flex items-center justify-center text-xs font-black text-sky-600 uppercase border border-sky-200">{c.author[0]}</div>
