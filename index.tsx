...
const uploadToCloudinary = async (file: File): Promise<string> => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(data.error?.message || 'Gagal upload ke server gambar.');
    }

    return data.secure_url;
  } catch (err: any) {
    console.error("Upload Catch:", err);
    throw new Error(err.message || 'Koneksi ke server gambar terputus.');
  }
};
...
const App = () => {
  ...
  const [showModal, setShowModal] = useState<{show: boolean, type: 'create' | 'gate'}>({ 
  ...
  const saveIndex = async (data: AlbumMetadata[]) => {
    ...
    try {
      ...
      console.error("Save index failed");
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const saveAlbumPhotos = async (albumId: string, photos: Photo[]) => {
    ...
    try {
      ...
      alert("Gagal memperbarui album di server.");
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const handleAddPhotos = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsUploading(true);
    // Explicitly casting to File[] to resolve 'unknown' type error in loops
    const files = Array.from(e.target.files) as File[];
    const newUploadedPhotos: Photo[] = [];

    try {
      // Proses upload satu per satu agar lebih stabil
      ...
      if (newUploadedPhotos.length > 0) {
        const updatedList = [...newUploadedPhotos, ...activePhotos];
        await saveAlbumPhotos(activeAlbumId, updatedList);
      }
    } catch (err: any) {
  ...
  const generateAICaption = async () => {
    ...
    try {
      ...
      // Call generateContent with model name and prompt/parts structure following standard examples
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: {
          parts: [
            { text: "Berikan 1 kalimat puitis singkat Bahasa Indonesia (max 10 kata) untuk foto kenangan ini." },
  ...
  const handleGate = (e: React.FormEvent) => {
    ...
    if (val.length < 2) return alert("Masukkan nama Anda.");
    
    const isNameAdmin = ADMIN_NAMES.map(n => n.toLowerCase()).includes(val.toLowerCase());
    ...
    if (isNameAdmin) {
      ...
      setIsAdmin(false);
      sessionStorage.setItem('isAdmin_v6', 'false');
    }
      ...
      <div className="fixed inset-0 flex items-center justify-center p-6 bg-sky-500">
        ...
        <div className="w-full max-w-md space-y-12 text-center z-10 animate-in fade-in zoom-in">
          ...
          <form onSubmit={handleGate} className="p-8 bg-white/90 backdrop-blur-3xl border border-white rounded-[3rem] shadow-2xl space-y-6">
            ...
            <button type="submit" className="w-full py-5 bg-sky-500 text-white font-black rounded-[2rem] shadow-xl hover:bg-sky-600 active:scale-95 transition-all flex items-center justify-center gap-3"><Zap className="w-4 h-4" /> Masuk ke Vault</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden text-sky-900 pb-32">
        ...
        <div className="max-w-7xl mx-auto flex flex-col gap-5 items-center">
          <div className="flex w-full justify-between items-center">
            <div className="flex items-center gap-4 cursor-pointer" onClick={() => {setCurrentView('home'); setActiveAlbumId(null);}}>
              ...
              <h1 className="text-lg font-serif font-black tracking-tight text-sky-700">Memoria</h1>
            </div>
            <div className="flex items-center gap-2">
               <button onClick={() => fetchIndex()} className={`p-2.5 text-sky-400 ${isSyncing ? 'animate-spin' : ''}`} title="Sinkronkan">
                 <RefreshCw className="w-5 h-5" />
               </button>
               <button onClick={() => { sessionStorage.clear(); window.location.reload(); }} className="p-2.5 text-red-400" title="Logout">
          ...
          <div className="flex items-center gap-2 overflow-x-auto w-full pb-1 no-scrollbar">
             <span className="text-[10px] font-black px-3 py-1.5 bg-sky-500 text-white rounded-full whitespace-nowrap shadow-md">
               {isAdmin ? 'Admin' : 'Tamu'}: {userName}
             </span>
             {currentView === 'home' && isAdmin && (
                <button onClick={() => { setInputVal(''); setShowModal({show: true, type: 'create'}); }} className="flex items-center gap-2 bg-white text-sky-600 border border-sky-100 px-4 py-1.5 rounded-full text-[10px] font-black shadow-sm uppercase whitespace-nowrap"><Plus className="w-3 h-3" /> Buat Album</button>
             )}
          </div>
        </div>
      </header>
        ...
        {currentView === 'home' ? (
          <div className="space-y-12 animate-in fade-in">
            <div className="px-2 text-white drop-shadow-lg">
              ...
              <p className="text-sm font-medium opacity-80">Menyimpan cerita indah di awan.</p>
            </div>
            ...
            {albums.length === 0 && !isSyncing ? (
               <div className="py-20 text-center bg-white/30 backdrop-blur-xl rounded-[3rem] border border-white/40">
                  ...
                  <p className="text-white font-black uppercase tracking-widest text-[10px]">Belum ada album.</p>
               </div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {albums.filter(a => a.name.toLowerCase().includes(searchTerm.toLowerCase())).map((album) => (
                  <div key={album.id} onClick={() => { setActiveAlbumId(album.id); setCurrentView('album'); fetchAlbumPhotos(album.id); }} className="group relative bg-white/70 backdrop-blur-3xl rounded-[2.5rem] overflow-hidden shadow-xl border border-white active:scale-95 transition-all cursor-pointer">
                    <div className="aspect-[4/5] bg-sky-50 relative flex items-center justify-center text-sky-200">
                      <ImageIcon className="w-12 h-12 opacity-20" />
                      <div className="absolute top-4 left-4"><div className="bg-sky-500 text-white px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest shadow-md">{album.photoCount} Momen</div></div>
                      {isAdmin && (
          ...
          <div className="space-y-10 animate-in fade-in">
            <div className="flex flex-col gap-6 items-center border-b border-white/60 pb-10">
              <div className="flex items-center gap-6 w-full">
                <button onClick={() => setCurrentView('home')} className="p-4 bg-white rounded-2xl text-sky-600 shadow-lg active:scale-90"><ArrowLeft className="w-6 h-6" /></button>
              ...
              {isAdmin && (
                <label className={`w-full cursor-pointer ${isUploading ? 'bg-sky-300 pointer-events-none' : 'bg-sky-500'} text-white py-4 rounded-2xl flex items-center justify-center gap-4 transition-all shadow-xl font-black text-xs uppercase tracking-widest`}>
                  {isUploading ? <RefreshCw className="w-5 h-5 animate-spin" /> : <Plus className="w-5 h-5" />} 
                  {isUploading ? 'Sedang Mengunggah...' : 'Tambah Foto Kenangan'}
            ...
            {isLoading ? (
              <div className="py-20 flex flex-col items-center justify-center text-white/50"><Loader2 className="w-12 h-12 animate-spin mb-4" /><p className="font-black uppercase tracking-widest text-[9px]">Membuka Loker...</p></div>
            ) : (
              <div className="columns-2 gap-4 space-y-4">
                {activePhotos.map((photo, idx) => (
                  <div key={photo.id} className="relative group break-inside-avoid bg-white/80 backdrop-blur-3xl rounded-[2rem] overflow-hidden shadow-lg border border-white transition-all active:scale-95 cursor-pointer" onClick={() => setSelectedPhotoIndex(idx)}>
        ...
        <div className="fixed inset-0 z-[60] bg-sky-900/60 backdrop-blur-3xl flex flex-col animate-in fade-in">
           ...
           <div className="w-full bg-white rounded-t-[3rem] p-8 space-y-6 shadow-2xl flex flex-col max-h-[60vh] overflow-hidden relative">
              ...
              <div className="relative p-5 bg-sky-50 rounded-2xl italic text-sky-900/70 text-sm font-serif leading-relaxed">
                 "{activePhotos[selectedPhotoIndex].caption || "Setiap gambar menceritakan ribuan kata."}"
                 {isAdmin && (
              ...
              <div className="flex-1 overflow-y-auto space-y-4 pr-2">
                 {activePhotos[selectedPhotoIndex].comments?.length === 0 ? (<p className="text-center text-sky-300 text-[9px] font-black uppercase py-4">Belum ada pesan.</p>) : 
              ...
              <div className="flex gap-3 mt-auto pt-2">
                 <input type="text" value={newComment} onChange={(e) => setNewComment(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addComment()} placeholder="Kirim pesan..." className="flex-1 px-5 py-4 bg-sky-50 border border-sky-100 rounded-2xl outline-none text-xs" />
      ...
      {/* Modal Create Album */}
      {showModal.show && showModal.type === 'create' && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-sky-900/40 backdrop-blur-2xl">
           <div className="bg-white border border-white w-full max-w-sm rounded-[3rem] shadow-2xl p-10 space-y-8 animate-in zoom-in">
              <div className="flex items-center gap-5"><div className="p-4 bg-sky-500 text-white rounded-2xl shadow-lg"><Plus className="w-6 h-6" /></div><h3 className="text-2xl font-serif font-black text-sky-900">Album Baru</h3></div>
              <form onSubmit={handleCreateAlbum} className="space-y-6">
                 <input autoFocus type="text" value={inputVal} onChange={(e) => setInputVal(e.target.value)} placeholder="Nama Album" className="w-full px-6 py-4 bg-sky-50 border border-sky-100 rounded-2xl font-bold text-sky-900 outline-none" />
                 <div className="flex flex-col gap-3 pt-4">
                    <button type="submit" className="w-full py-5 bg-sky-500 text-white font-black rounded-2xl shadow-xl active:scale-95 uppercase text-[10px] tracking-widest">Buat Sekarang</button>
                    <button type="button" onClick={() => setShowModal({show: false, type: 'gate'})} className="text-sky-400 font-bold text-[10px] uppercase text-center">Batal</button>
                 </div>
              </form>
           </div>
        </div>
